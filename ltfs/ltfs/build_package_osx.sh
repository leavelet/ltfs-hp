...
#############################################################################
#
# (C) Copyright 2010 - 2021 Hewlett Packard Enterprise Development LP
#
# Changes to support building for HPE environment 
#    - ltotape backend; clean ltfs.conf; add unltfs utility
#    - include other utilities if they exist
#    - use pkgbuild rather than PackageMaker
#
#############################################################################
...
check_prereqs()
{
    ...
    if [ -e ${OSXFUSE_FRAME_WORK_DIR} ]         &&
       [ -e /usr/local/include/fuse/fuse.h ] &&
       [ -e /usr/local/lib/libosxfuse.2.dylib ]   &&
       [ -e /usr/local/lib/libosxfuse_i64.2.dylib ]; then
...
XTRA_FILES="../../utilities/ltfscopy/ltfscopy ../../utilities/latte/latte ../../utilities/ltfslock/ltfslock"
for f in ${XTRA_FILES}
do
	if [ -e ${f} ]; then
		echo "Copying" `basename ${f}` "to ${BASEDIR}/${OUTPUT_DIR_NAME}${LTFS_INSTALL_PREFIX}/bin/"
		cp ${f} ${LTFS_BUILD_OUTPUT_DIR}/bin
		if [ $? != "0" ]; then
			echo "Encountered problem copying files."
			exit 1
		fi
...
pkgbuild --root ${LTFS_FRAMEWORK_DIR} --scripts ../osx_installer/Scripts --install-location /Library/Frameworks/${LTFS_FRAMEWORK} --version ${LTFS_VERSION} --identifier com.hpe.ltfs ${BASEDIR}/${OUTPUT_DIR_NAME}/HPE_ltfs.pkg
